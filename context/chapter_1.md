...
#### <mark>1.9.2 并发与并行</mark>

- **驱动进步的动力**：数字计算机的历史始终受到“做更多事”和“运行更快”这两个需求的持续驱动。
- **核心概念区分**：**并发**是一个通用的概念，指一个同时具有多个活动的系统；而**并行**则是指用并发来使一个系统运行得更快。
###### 层次一：线程级并发（Thread-Level Concurrency）

- **基本概念**：该层次构建在进程抽象之上，允许在一个进程中执行多个控制流。
- **演进历程**：传统的单处理器系统通过在进程间快速切换来“模拟”并发。直到最近随着**多核处理器**和**超线程**技术的出现，真正的多处理器系统才变得普及。
- **多核处理器**：将多个 CPU（核）集成到一个集成电路芯片上。典型结构中每个核拥有独立的 L1 和 L2 高速缓存，并共享更高层次的高速缓存和主存接口。
- **超线程（同时多线程）**：允许单个 CPU 在单个时钟周期的基础上决定执行哪一个线程。它通过备份程序计数器和寄存器文件等硬件，规避了常规处理器线程切换时所需的大约 20,000 个时钟周期的巨大开销。例如，一个 4 核且支持超线程的 Intel Core i7 处理器实际上可以并行执行 8 个线程。

###### 层次二：指令级并行（Instruction-Level Parallelism）

- **执行速率飞跃**：在较低的抽象层次上，处理器可以同时执行多条指令。早期的微处理器（如 1978 年的 Intel 8086）执行一条指令需要多个时钟周期，而现代处理器可以保持每个时钟周期执行 2~4 条指令的速率。
- **流水线技术（Pipelining）**：将执行一条指令所需的活动划分成不同的步骤，由处理器的硬件组织成一系列阶段来并行操作不同的指令，从而让处理器的执行速率接近于每个时钟周期一条指令。
- **超标量处理器（Superscalar）**：当处理器能够达到比每个周期一条指令更快的执行速率时，就被称为超标量处理器，大多数现代处理器均支持这一模型。
###### 层次三：单指令、多数据并行（SIMD 并行）

- **最低层次的并行**：在最低抽象层次上，现代处理器拥有特殊的硬件，允许一条指令产生多个可以并行执行的操作。
- **典型应用**：较新几代的 Intel 和 AMD 处理器都具有并行对 8 对单精度浮点数做加法的 SIMD 指令，常用于加速图像、声音和视频数据应用的执行速度。
- **实现方式**：除了依靠编译器自动抽取 SIMD 并行性外，更可靠的方法是利用特殊向量数据类型（如 GCC 支持的向量数据类型）直接编写程序。

#### 1.9.3 计算机系统中抽象的重要性

<details>
<summary>图1-18</summary>
<img src="../images/ch1/1-18.png">
</details>

###### 抽象的核心价值与定义
- **简化复杂性**：抽象允许程序员只需了解简单的应用程序接口（API）即可直接使用代码，而无需了解其内部精细的工作原理。
- **语言层面的支持**：不同的编程语言提供了不同形式和等级的抽象支持，例如 Java 的类声明以及 C 语言的函数原型。
###### 硬件层的抽象：指令集架构（ISA）
- **提供统一模型**：指令集架构提供了对实际处理器硬件的抽象。
- **“串行执行”的假象**：在此抽象下，机器代码程序表现得好像运行在一个**一次只执行一条指令**的处理器上。
- **屏蔽底层并行性**：尽管底层的真实硬件远比抽象描述的更复杂精细（会并行地执行多条指令），但它的执行结果总是与那个简单有序的串行模型保持一致。这使得不同的处理器实现（不同的开销和性能）能够运行完全相同的机器代码。
###### 计算机系统提供的分层抽象视图（图 1-18）：
- **文件（Fields）** $\rightarrow$ 对 **I/O 设备** 的抽象表示。
- **虚拟内存（Virtual Memory）** $\rightarrow$ 对 **程序存储器（主存）** 的抽象表示。
- **指令集架构（ISA）** $\rightarrow$ 对 **处理器** 的硬件抽象。
- **进程（Processes）** $\rightarrow$ 对整个 **[处理器 + 主存 + I/O设备]**（即正在运行的程序）的整体抽象。
- **操作系统（OS）** $\rightarrow$ 统一管理并提供上述所有底层硬件抽象的软件层。
###### 更高级别拓扑：虚拟机（Virtual Machine）

在上述三大操作系统抽象之上，书中引入了一个更高维度的全新抽象
- **对整个计算机的抽象**：虚拟机提供了对**包括操作系统、处理器和程序在内的整台计算机**的完整抽象。
- **历史与现代价值**：该思想由 IBM 在 20 世纪 60 年代提出 sales，但在现代彰显出巨大优势。它完美解决了现代计算中“必须在同一台机器上运行为不同操作系统（如 Windows、macOS、Linux）或同一操作系统不同版本设计的程序”的隔离与兼容需求。